...
bool g_orb_walking = false;
bool g_kiting = false;
bool OnStartMessage = false;
int g_nLastAttackCmdTick = 0;
int g_nLastMoveCmdTick = 0;
int g_nLastAttackCmdTick_K = 0;
int g_nLastMoveCmdTick_K = 0;

int KEY_ORB = VK_CONTROL;
int KEY_KITING = VK_SPACE;
int KEY_PRESSED = 0;
...
HRESULT WINAPI Hooked_Present(LPDIRECT3DDEVICE9 Device, CONST RECT* pSrcRect, CONST RECT* pDestRect, HWND hDestWindow, CONST RGNDATA* pDirtyRegion)
{
	...
	if (ImGui_ImplWin32_Init(g_hwnd))
	{
		if (ImGui_ImplDX9_Init(Device))
		{
			if (g_menu_opened)
			{
				ImGui::Begin("thxmxx :)", &g_menu_opened, ImGuiWindowFlags_NoSavedSettings);
				{
					ImGui::BeginChild("##child", ImVec2(450.0f, 450.0f), false, ImGuiWindowFlags_NoSavedSettings);
					{
						...
						ImGui::Checkbox("Orbwalker", &g_orb);
						ImGui::Checkbox("Exit", &g_unload);
					}
					ImGui::EndChild();
				}
				ImGui::End();
		...
		while (obj)
		{
			if (obj->IsHero() && obj->IsEnemyTo(me))
			{
				if (obj->GetHealth() > .0f)
				{
					...
					//char buff[100];
					//float flTimeToMove = me->GetAttackCastDelay();
					//int nTicksToMove = (int)(flTimeToMove * 1000);

					//float flTimeTilAttack = me->GetAttackDelay();
					//int nTicksTilAttack = (int)(flTimeTilAttack * 1000);
					//sprintf(buff, "Attacks: %f %f KEYPRESSED: %d g_kiting %d", obj->GetBaseAttackDamage(), obj->GetBonusAttackDamage(), KEY_PRESSED, g_kiting);

					//render.draw_text(objpos_w2s.X, objpos_w2s.Y + 60, buff, true, ImColor(255, 0, 0, 255));
				}
			}
	...
	if (g_orb == true) {
		if (g_orb_walking)
		{
			...
			if (m_orbTarget != nullptr) {
				float flTimeTilAttack = me->GetAttackDelay();
				int nTicksTilAttack = (int)(flTimeTilAttack * 1000);
				if ((int)(Engine::GetGameTime() * 1000) >= (g_nLastAttackCmdTick + nTicksTilAttack))
				{
					Engine::AttackTarget(m_orbTarget);
					g_nLastAttackCmdTick = (int)(Engine::GetGameTime() * 1000);
				}
				else
				{
					float flTimeToMove = me->GetAttackCastDelay();
					...
					if ((int)(Engine::GetGameTime() * 1000) >= (g_nLastAttackCmdTick + nTicksToMove) && (int)(Engine::GetGameTime() * 1000) >= g_nLastMoveCmdTick + 60)
					{
						Engine::MoveTo(&Engine::GetMouseWorldPosition());
						g_nLastMoveCmdTick = (int)(Engine::GetGameTime() * 1000);
					}
				}
				auto color = createRGB(255, 0, 0);
				Functions.DrawCircle(&m_orbTarget->GetPos(), m_orbTarget->GetBoundingRadius(), &color, 0, 10.0f, 0, .8f);
			...
			{
				int nTicksToMove = 60;
				if ((int)(Engine::GetGameTime() * 1000) >= (g_nLastMoveCmdTick + nTicksToMove))
				{
					Engine::MoveTo(&Engine::GetMouseWorldPosition());
					g_nLastMoveCmdTick = (int)(Engine::GetGameTime() * 1000);
				}
				g_nLastAttackCmdTick = 0;
			}
		}
		...
		if (g_kiting)
		{
			...
			if (m_KittingTarget != nullptr) {
				float flTimeTilAttack = me->GetAttackDelay();
				int nTicksTilAttack = (int)(flTimeTilAttack * 1000);
				if ((int)(Engine::GetGameTime() * 1000) >= (g_nLastAttackCmdTick_K + nTicksTilAttack))
				{
					Engine::AttackTarget(m_KittingTarget);
					g_nLastAttackCmdTick_K = (int)(Engine::GetGameTime() * 1000);
				}
				else
				{
					float flTimeToMove = me->GetAttackCastDelay();
					int nTicksToMove = (int)(flTimeToMove * 1000) + 55;

					if ((int)(Engine::GetGameTime() * 1000) >= (g_nLastAttackCmdTick_K + nTicksToMove) && (int)(Engine::GetGameTime() * 1000) >= g_nLastMoveCmdTick_K + 60)
					{
						Engine::MoveTo(&Engine::GetMouseWorldPosition());
						g_nLastMoveCmdTick_K = (int)(Engine::GetGameTime() * 1000);
					}
				}
				auto color = createRGB(255, 0, 0);
				Functions.DrawCircle(&m_KittingTarget->GetPos(), m_KittingTarget->GetBoundingRadius(), &color, 0, 10.0f, 0, .8f);
			...
			{
				int nTicksToMove = 60;
				if ((int)(Engine::GetGameTime() * 1000) >= (g_nLastMoveCmdTick_K + nTicksToMove))
				{
					Engine::MoveTo(&Engine::GetMouseWorldPosition());
					g_nLastMoveCmdTick_K = (int)(Engine::GetGameTime() * 1000);
				}
				g_nLastAttackCmdTick_K = 0;
			}
		}
	}
